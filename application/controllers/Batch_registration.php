...
class Batch_registration extends CI_Controller
{
    ...
    function show_data()
    {
        ...
        $classes = [
            1 => $this->csvToArray('assets/1.csv'),
            2 => $this->csvToArray('assets/2.csv'),
            3 => $this->csvToArray('assets/3.csv'),
            4 => $this->csvToArray('assets/4.csv'),
            5 => $this->csvToArray('assets/5.csv'),
            6 => $this->csvToArray('assets/6.csv'),
            7 => $this->csvToArray('assets/7.csv'),
            8 => $this->csvToArray('assets/8.csv'),
        ];
    ...
    function check_register($data, $class, $sectionsArr)
    {
        ...
        foreach ($data as $row) {
            $student_data['name'] = $row['STUDENT NAME'];
            $student_data['mobile'] = '';
            $student_data['email'] = $row['EMAIL ID'];
            $student_data['password'] = 'orange@2024';
            $student_data['pin'] = '123456';
            $student_data['school_name'] = " ";
            $student_data['address'] = ' ';
            $student_data['stu_teacher_id'] = $row['Teacher Code'];
            $student_data['city'] = ' ';
            $student_data['state'] = ' ';
            ...
            // $this->db->where('email', $row['EMAIL ID']);
            // $this->db->delete('web_user');

            $check = $this->WebModel->validate_email($row['EMAIL ID']);
            if (!empty($check)) {
                // array_push($unregistered_students, $student_data);
                echo '<pre>', $row['EMAIL ID'], '</pre>';
            } else {
                $res = $this->add_student_custom($student_data);
                if ($res) {
                    echo '<pre>Registered :', $row['EMAIL ID'], '</pre>';
                }
            }
        }
