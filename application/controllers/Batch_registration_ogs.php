...
class Batch_registration_ogs extends CI_Controller
{
  ...
  function show_data()
  {
    ...
    $classes = [
      7 => $this->csvToArray('assets/ogs7.csv'),
      8 => $this->csvToArray('assets/ogs8.csv'),
    ];
  ...
  function check_register($data, $class, $sectionsArr)
  {
    ...
    foreach ($data as $row) {
      ...
      $student_data['school_name'] = "Oxford Grammar School";
      $student_data['address'] = ' ';
      $student_data['stu_teacher_id'] = '5B24AM09KG4';
      $student_data['city'] = ' ';
      $student_data['state'] = ' ';
      ...
      // echo '<pre>', var_dump($student_data), '</pre';
      // exit;
  ...
  {
    // Load your CSV data
    $csvData = $this->csvToArray('assets/ogs7.csv');
    $csvData = array_merge($csvData, $this->csvToArray('assets/ogs8.csv'));
    ...
    foreach ($csvData as $row) {
      ...
      if (!empty($name) && !empty($email)) {
        ...
        $this->db->update('web_user', ['fullname' => $name]);

        if ($this->db->affected_rows() > 0) {
          echo "<pre>Updated name for: $email</pre>";
        } else {
          echo "<pre>Failed to update name for: $email</pre>";
        }
      }
    }
  }
