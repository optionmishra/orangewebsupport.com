					...
					<div class="row">
						<div class="col-lg-12">
							<form method="post" action="<?= base_url('admin_master/add_assigntest') ?>">
								...
								<div class="row">
									<div class="col-lg-2">
										<h6 class="sp-text-subhdg">Select Series</h6>
										<select class="form-control w-100" name="series" id="series_name" required>
...
<script>
	$(document).ready(function() {
		...
		$('#series_name').on('change', e => {
			$('#class_name').prop('selectedIndex', 0);
			$('#section_name').prop('selectedIndex', 0);
			$('#paper_type').prop('selectedIndex', 0);
		});
		// get sections of selected class
		$('#class_name').on('change', e => {
			// fetch sections based on selected class
			const series_id = $('#series_name').val();
			...
			if (class_id) {
				...
				// Paper Set
				fetch('<?= base_url() . 'admin_master/get_paper_set/' ?>' + class_id + `/${series_id}`)
					.then(response => response.json())
					.then(data => {
						// put them in selection
						if (data.length) {
							$('#testWarningText').remove();
							data.forEach(item => {
								$('#paper_type').append(`<option value="${item.type}">${item.name} (${item.description})</option>`)
							});
						} else {
							// $('#paper_type').prop('disabled', true);
							$('#paper_type').parent().append('<p id="testWarningText" class="text-danger p-2">Test not available for the selected book, please choose different book<p>')
							$('#paper_type').attr('title', 'Test not available for the selected book, please choose different book')
							$('#paper_type').append(`<option value="" disabled>Test not available for the selected book, please choose different book</option>`)
						}
					});
			}
		})
