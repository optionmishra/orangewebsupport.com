...
<head>
    ...
    <title>Missing Websupport</title>
...
<body>
    ...
    $sql = "SELECT c.id AS category_id, c.name AS category_name, s.id AS subject_id, s.name AS subject_name FROM category c CROSS JOIN subject s WHERE NOT EXISTS (SELECT 1 FROM websupport w WHERE w.type = c.id AND w.subject = s.id) AND s.sid = $main_subject_id ORDER BY c.id";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        echo "<table class='table table-striped table-bordered' id='myTable'><thead><tr><th>Serial</th><th>Category</th><th>Missing Subjects Name</th></tr></thead><tbody>";
