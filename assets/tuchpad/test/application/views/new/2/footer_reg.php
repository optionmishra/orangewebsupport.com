...
<section class="">
  ...
  <div class="col-sm-12 col-md-4 footer-pdg">
          <ul class="footer-links text-center nw-footer-list">
              <li><img class="nw-img" src="<?php echo base_url(); ?>assets/footer_img/logos/orangefull.svg"></li>
        ...
        <div class="col-sm-12 col-md-4 footer-pdg">
         <ul class="footer-links text-right ft-lst">
              <li class="li-list-nw"><img class="nw-ft-img" src="<?php echo base_url(); ?>assets/footer_img/logos/touchpadfooterright.svg"></li>
              <li class="li-list-nw">
  ...
  <script>
    ...
    $('#board').on('change', function(e) {
      $('#seriesClassesContainer').html(`<div class="col-lg-12 p-2"><select class="form-control series-select" id="series"><option value="" disabled selected>Select Series</option></select></div>`);
    ...
    $('#seriesClassesContainer').on('change', 'div select', function(e) {
      ...
      $(this).parent().append(`<div class="row m-0 series-classes-container pb-2 justify-content-center" id="${classesContainerID}"></div>`);
      ...
      fetch(url).then(res => res.json()).then(data => {
        ...
        $('#' + classesContainerID).append(classesCheckBox);
        // console.log($('#' + classesContainerID + ' span').length);
        if ($('#' + classesContainerID + ' span').length == 1) {
          $('#' + classesContainerID).html('<p class="text-danger">No classes available for this series.</p>');
        };
        // $('#addSeries').off('click');
      });
    });
    $('#seriesClassesContainer').on('click', 'div #addSeries', () => {
      ...
      let seriesClassesElement = `<div class="col-lg-12 p-2"><input type="hidden" name="series[]" value="${seriesValue}"><select class="form-control series-select" name="series[]" required="true" disabled><option value="${seriesValue}" selected>${seriesName}</option></select><div class="row m-0 series-classes-container pb-2">`;
      currCheckedClasses.forEach(currClass => {
        seriesClassesElement += `<span class="col-md-2"><input type="hidden" name="${seriesValue}Classes[]" value="${currClass}"><input class="m-1" type="checkbox" name="${seriesValue}Classes[]" value="${currClass}" disabled="" checked><label class="m-1">Class ${currClass}</label></span>`;
      })
